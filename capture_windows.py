...
✓ Live adjustable brightness, contrast, saturation, gain (via OpenCV properties)
✓ MP4 recording
...
# ============ GUI Application ============
class CaptureApp:
    """Windows UVC Camera GUI with full camera control."""
    ...
    def __init__(self, root):
        ...
        self.format_var = tk.StringVar(value="MJPG")
    ...
    def open_camera(self):
        ...
        # Use MSMF or DSHOW backend on Windows
        self.cam = Camera(
        ...
        # Load camera control ranges from OpenCV properties
    ...
    def _load_camera_control_ranges(self):
        ...
        for ctrl_name in controls_to_check:
            ...
            try:
                ...
                # Note: OpenCV doesn't provide min/max easily, so we use our defaults
                if current is not None and current >= 0:
                    # Try to get reasonable defaults (0-255 is typical)
                    # If the current value is outside 0-255, adjust range
                    if ctrl_name in self.control_vars:
                        var = self.control_vars[ctrl_name]
                        slider = None
                        # Find the slider widget and update it
                        ...
                        if slider:
                            ...
                            # Most cameras use 0-255, but we'll keep defaults for now
                            pass
