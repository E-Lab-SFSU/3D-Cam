...
# -*- coding: utf-8 -*-
"""
Pair Detector YOLO — YOLO-based object detection for pair tracking

This app lets you:
  • Load a YOLO model (.pt file) for object detection
  • Open a video file (persisted in preset)
  • Preview at ~15 FPS and click to set the optical center (live re-clickable)
  • YOLO detects objects per frame, using bounding box centers as blob centers
  • Area is calculated from thresholded white pixels within each bounding box
  ...
  • Save/restore default settings + optical center + last video path to detect_pairs_yolo_default.json

NOTES
  - "Tracked" OpenCV window: click once to update the optical center (yellow crosshair)
  - YOLO model must be loaded before processing
  - Bounding box centers from YOLO become blob centers
  - Area is calculated by thresholding the region within each bounding box
  - Threshold, blur, and invert threshold parameters apply to area calculation within bounding boxes
...
try:
    ...
    YOLO = None
    print("[WARN] ultralytics not available. YOLO functionality will be limited.")

# ============ Persistent default settings file ============
# This file stores default settings that load when detect_pairs_yolo.py starts or when a new video is opened.
# For reprocessing videos, use the detect_pairs_yolo_preset.json files in video folders (saved during export).
PRESET_PATH = "detect_pairs_yolo_default.json"
...
DEFAULT_PARAMS = {
    ...
    "yolo_confidence":    0.25,   # YOLO confidence threshold (0.0-1.0)
    "yolo_model_path":    "",     # Path to YOLO model file (.pt)
}
...
# ============ YOLO model state ============
yolo_model = None                      # Loaded YOLO model instance
...
def download_yolo_model(model_name: str) -> Optional[str]:
    """
    Download a YOLO model by name (e.g., 'yolov8n.pt').
    Returns the model name (which YOLO will resolve to cached path), or None on failure.
    Ultralytics automatically handles caching - we can just use the model name.
    """
    if YOLO is None:
        messagebox.showerror("Ultralytics Not Available", "ultralytics package is not installed.\n\nPlease install it with: pip install ultralytics")
        return None
    
    try:
        print(f"[INFO] Downloading YOLO model: {model_name}")
        
        # Create a YOLO instance with the model name
        # This will automatically download the model if not already cached
        # and store it in ~/.ultralytics/weights/ or similar
        ...
        model_path = None
        # Try to find the actual cached file
        if ultralytics_cache.exists():
            # Check direct location
            model_file = ultralytics_cache / model_name
            if model_file.exists():
                model_path = str(model_file)
            else:
                # Check subdirectories (versioned cache)
                for item in ultralytics_cache.iterdir():
                    if item.is_dir():
                        candidate = item / model_name
                        if candidate.exists():
                            model_path = str(candidate)
                            break
        
        # If we found a cached path, use it; otherwise use the model name
        # (YOLO will resolve the name to the cached file automatically)
        result_path = model_path if model_path else model_name
        
        print(f"[INFO] YOLO model downloaded and cached: {result_path}")
        return result_path
    except Exception as e:
        messagebox.showerror("Download Failed", f"Failed to download YOLO model:\n{e}")
        print(f"[ERROR] Failed to download YOLO model: {e}")
        return None

def load_yolo_model():
    """Load YOLO model from path or model name specified in params."""
    global yolo_model, params
    
    if YOLO is None:
        messagebox.showerror("Ultralytics Not Available", "ultralytics package is not installed.\n\nPlease install it with: pip install ultralytics")
        return False
    
    model_path = params.get("yolo_model_path", "").strip()
    if not model_path:
        messagebox.showwarning("No Model Path", "Please specify a YOLO model path or model name first.\n\nYou can download a model using the dropdown above, or browse to an existing .pt file.")
        return False
    
    # YOLO can handle both file paths and model names (e.g., 'yolov8n.pt')
    # If it's a file path, check if it exists
    # If it's a model name, YOLO will download it automatically if not cached
    if os.path.isabs(model_path) or (os.path.sep in model_path and os.path.exists(model_path)):
        # It's a file path - verify it exists
        if not os.path.exists(model_path):
            messagebox.showerror("File Not Found", f"YOLO model file not found:\n{model_path}")
            return False
    # Otherwise, treat it as a model name (YOLO will handle download/cache)
    
    try:
        yolo_model = YOLO(model_path)
        print(f"[INFO] YOLO model loaded: {model_path}")
        display_name = os.path.basename(model_path) if os.path.sep in model_path else model_path
        messagebox.showinfo("Model Loaded", f"YOLO model loaded successfully:\n{display_name}")
        return True
    except Exception as e:
        messagebox.showerror("Load Failed", f"Failed to load YOLO model:\n{e}")
...
def optimize_optical_center():
    ...
    while True:
        ...
        _, binary = cv2.threshold(blur, int(p["threshold"]), 255, thresh_type)
                ...
                # YOLO detection - use background-subtracted BGR frame
        # Convert gray_contrast to BGR for YOLO
        frm_bgsub_bgr = cv2.cvtColor(gray_contrast, cv2.COLOR_GRAY2BGR)
        if yolo_model is None:
            blobs = []
        else:
            blobs = detect_yolo(frm_bgsub_bgr, yolo_model, test_cx, test_cy, p, 
                                confidence_threshold=float(p.get("yolo_confidence", 0.25)))
...
def export_video():
    ...
    while True:
        ...
        _, binary = cv2.threshold(blur, int(p["threshold"]), 255, thresh_type)
                ...
                # YOLO detection + pairing - use background-subtracted BGR frame
        # Convert gray_contrast to BGR for YOLO
        frm_bgsub_bgr = cv2.cvtColor(gray_contrast, cv2.COLOR_GRAY2BGR)
        if yolo_model is None:
            blobs = []
        else:
            blobs = detect_yolo(frm_bgsub_bgr, yolo_model, xCenter, yCenter, p,
                                confidence_threshold=float(p.get("yolo_confidence", 0.25)))
        method = p.get("pair_method", "Hungarian")
        if method == "Greedy":
...
def preview_loop():
    ...
    try:
        ...
        if yolo_model is None:
            ...
            blobs = detect_yolo(frm_bgsub_bgr, yolo_model, xCenter, yCenter, params,
                                confidence_threshold=float(params.get("yolo_confidence", 0.25)))
...
def main():
    ...
    # Update window title for YOLO version
    from lib.gui import format_window_title
    root.title(format_window_title("Pair Detector YOLO", version="v1.0"))
    
    # Add YOLO-specific controls
    try:
        from lib.gui import STANDARD_PADDING
        padding = STANDARD_PADDING.get("small", 5)
    except:
        padding = 5
    
    content_frame = root.children['!frame'].children['!frame']  # Get the main content frame
    
    # YOLO Model frame
    frm_yolo = ttk.LabelFrame(content_frame, text="YOLO Model", padding=padding)
    # Find the last row in content_frame by checking grid info
    last_row = 0
    for child in content_frame.winfo_children():
        info = child.grid_info()
        if info:
            row = info.get('row', 0)
            if isinstance(row, int) and row > last_row:
                last_row = row
    frm_yolo.grid(row=last_row + 1, column=0, sticky="ew", padx=padding, 
                  pady=(0, padding))
    frm_yolo.grid_columnconfigure(1, weight=1)
    
    # Download model dropdown and button
    ttk.Label(frm_yolo, text="Download Model:").grid(row=0, column=0, sticky="w", padx=4, pady=2)
    ...
    available_models = [
        ...
        "yolov10b.pt", # YOLOv10 base
        "yolov10l.pt", # YOLOv10 large
        "yolov10x.pt", # YOLOv10 extra large
    ]
    
    var_download_model = tk.StringVar(value="yolov8n.pt")
    cmb_download = ttk.Combobox(frm_yolo, textvariable=var_download_model, values=available_models, 
                                state="readonly", width=20)
    cmb_download.grid(row=0, column=1, sticky="w", padx=4, pady=2)
    widgets["cmb_yolo_download"] = cmb_download
    
    def handle_download_model():
        selected_model = var_download_model.get()
        ...
        y = (download_window.winfo_screenheight() // 2) - (100 // 2)
        download_window.geometry(f"400x100+{x}+{y}")
        
        label = tk.Label(download_window, text=f"Downloading {selected_model}...\nThis may take a minute.", 
                        font=("Arial", 10))
        label.pack(pady=20)
        download_window.update()
        
        # Download the model
        model_path = download_yolo_model(selected_model)
        download_window.destroy()
        
        if model_path:
            # Update model path entry
            var_model_path.set(model_path)
            params["yolo_model_path"] = model_path
            messagebox.showinfo("Download Complete", 
                              f"Model downloaded successfully!\n\nPath: {model_path}\n\nWould you like to load it now?",
                              parent=root)
            # Optionally auto-load
            response = messagebox.askyesno("Load Model?", "Load the downloaded model now?", parent=root)
            if response:
                load_yolo_model()
    
    btn_download = ttk.Button(frm_yolo, text="Download", command=handle_download_model)
    btn_download.grid(row=0, column=2, padx=4, pady=2)
    widgets["btn_yolo_download"] = btn_download
    
    # Model path entry and browse button
    ttk.Label(frm_yolo, text="Model Path:").grid(row=1, column=0, sticky="w", padx=4, pady=2)
    var_model_path = tk.StringVar(value=params.get("yolo_model_path", ""))
    entry_model = ttk.Entry(frm_yolo, textvariable=var_model_path, width=40)
    entry_model.grid(row=1, column=1, sticky="ew", padx=4, pady=2)
    widgets["entry_yolo_model"] = entry_model
    gui_vars_numeric["yolo_model_path"] = var_model_path
    ...
    def browse_model():
        ...
        if path:
            ...
            params["yolo_model_path"] = path
    
    btn_browse = ttk.Button(frm_yolo, text="Browse...", command=browse_model)
    btn_browse.grid(row=1, column=2, padx=4, pady=2)
    widgets["btn_yolo_browse"] = btn_browse
    
    # Load model button
    btn_load_model = ttk.Button(frm_yolo, text="Load YOLO Model", command=load_yolo_model)
    btn_load_model.grid(row=2, column=0, columnspan=3, sticky="ew", padx=4, pady=2)
    widgets["btn_yolo_load"] = btn_load_model
    
    # Confidence threshold slider
    ttk.Label(frm_yolo, text="Confidence:").grid(row=3, column=0, sticky="w", padx=4, pady=2)
    var_conf = tk.DoubleVar(value=params.get("yolo_confidence", 0.25))
    scale_conf = ttk.Scale(frm_yolo, from_=0.0, to=1.0, variable=var_conf, orient=tk.HORIZONTAL)
    scale_conf.grid(row=3, column=1, sticky="ew", padx=4, pady=2)
    widgets["scale_yolo_conf"] = scale_conf
    gui_vars_numeric["yolo_confidence"] = var_conf
    
    label_conf_val = ttk.Label(frm_yolo, text="0.25")
    label_conf_val.grid(row=3, column=2, padx=4, pady=2)
    widgets["label_yolo_conf_val"] = label_conf_val
    
    def update_conf_label(val):
        label_conf_val.config(text=f"{float(val):.2f}")
        params["yolo_confidence"] = float(val)
    
    scale_conf.configure(command=update_conf_label)
    
    # Update params when model path changes
    def update_model_path(*args):
        params["yolo_model_path"] = var_model_path.get()
    
    var_model_path.trace_add("write", update_model_path)
