    ...
    def load_csv_file(self, file_path: str):
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                # Check required columns - allow X,Y in pixels or mm, but need either Z_mm or Zprime_mm
                required_cols = ['Frame_Number', 'Track_ID']
                if not all(col in reader.fieldnames for col in required_cols):
                    messagebox.showerror("Error", "CSV file must contain Frame_Number and Track_ID columns")
                    return
                
                # Check for coordinate columns - prefer calibrated columns
                ...
                for row in all_rows:
                    try:
                        ...
                        # Get Z coordinate - prefer calibrated, fall back to calculated values
                        z = 0.0
                        z_str = ''
                        
                        # Try calibrated Z_mm first
                        if has_z_mm:
