...
### Why can't I just type `run_visualize3d.sh`?
...
Instead of typing:
```bash
chmod +x run_visualize3d.sh
chmod +x run_pair_detect.sh
...
If you really want to type just `run_visualize3d.sh` (without `./`), you can:

1. **Add current directory to PATH** (not recommended for security):
   ```bash
   export PATH=$PATH:.
   run_visualize3d.sh  # Now works without ./
   ```

2. **Or use the full path** (works from anywhere):
   ```bash
   /home/robocam/Github/3d-cam/run_visualize3d.sh
   ```

But `./run_visualize3d.sh` is the standard, safe way to do it.
